...
const FacultyMentorTeams = () => {
  ...
  const handleLogInteraction = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `/api/faculty/mentor/team/${interactionTeam.id}/interaction`,
        { records },
        {
          headers: { Authorization: `Bearer ${userInfo?.token}` },
        },
  ...
  const fetchVenue = async () => {
    try {
      ...
      if (res.data.availableDate) {
        setAvailableDate(
          new Date(res.data.availableDate).toISOString().split("T")[0],
        );
      }
  ...
  const handleUpdateVenue = async (e) => {
    ...
    try {
      ...
      const payload = { venue };
      if (availableDate)
        payload.availableDate = new Date(availableDate).toISOString();
  ...
  const handleGradeSubmit = async (e) => {
    ...
    try {
      const userInfo = JSON.parse(localStorage.getItem("userInfo"));
      await axios.post(
        `/api/faculty/mentor/grade/${selectedSubmission.id}`,
        {
  ...
  const uniquePbls = [
    ...new Map(teams?.map((team) => [team.pbl.id, team.pbl]) || []).values(),
  ];
  ...
  const filteredTeams = teams.filter((team) => {
    ...
    const matchesLeader =
      team.leader?.user?.name?.toLowerCase().includes(q) ||
      team.leader?.enrollmentNumber?.toLowerCase().includes(q);
        ...
        <div className="text-center p-8 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
          <p className="text-gray-500">
            {searchQuery
              ...
              : "No teams found for the selected PBL."}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {filteredTeams?.map((team) => {
            ...
            return (
              <div
                key={team.id}
                className="bg-white dark:bg-gray-800 shadow-sm rounded-2xl border border-gray-100 dark:border-gray-700 p-6 hover:shadow-md transition-shadow"
              >
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        ...
                        {isSuperMentorApproved
                          ? "Super Mentor: Approved"
                          : isSuperMentorRejected
                            ? "Rejected (Needs New Idea)"
                            : "Super Mentor: Pending"}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500 mt-1">
                      {team.pbl.subject} (Sem {team.pbl.semester})
                    </p>
                    <p className="text-xs text-indigo-600 dark:text-indigo-400 mt-0.5">
                      Mentor ID:{" "}
                      {team.pbl.pblFaculties?.[0]?.mentorIdFormatted || "N/A"}
                    </p>
                  </div>
                  <div className="text-right text-sm flex flex-col items-end">
                    <p className="font-semibold text-gray-800 dark:text-white">
                      Leader: {team.leader?.user?.name}
                    </p>
                  ...
                  <div className="mb-4 p-3 rounded-xl border text-xs leading-relaxed bg-red-50 dark:bg-red-900/20 border-red-200 text-red-800 dark:text-red-300">
                    ...
                    <p>
                      ...
                      {team.superMentorFeedback ||
                        "Scope / Idea needs revision"}
                      ". The team leader must submit a new project idea &
                      synopsis.
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-4">
                  ...
                  {team.submissions.length === 0 ? (
                    ...
                    team.submissions?.map((sub) => {
                      const phaseNum =
                        team.pbl.phases?.find((p) => p.id === sub.phaseId)
                          ?.phaseNumber || "1";
                      const isResubmitted =
                        sub.status === "PENDING" &&
                        sub.mentorGrades &&
                      ...
                      const isAwaitingSuperMentor =
                        sub.status === "AWAITING_SUPER_MENTOR";
                        ...
                        >
                          <div className="space-y-2 flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {isResubmitted ? (
                                ...
                                <span className="px-2.5 py-0.5 bg-purple-100 text-purple-800 dark:bg-purple-900/40 dark:text-purple-300 rounded-full text-xs font-bold uppercase flex items-center gap-1">
                                  <Clock className="w-3 h-3" /> Mentor Approved
                                  &bull; Awaiting Super Mentor
                                </span>
                              ) : sub.status === "PENDING" ? (
                                ...
                                <span className="px-2.5 py-0.5 bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300 rounded-full text-xs font-bold uppercase flex items-center gap-1">
                                  <CheckCircle2 className="w-3 h-3" /> Super
                                  Mentor Approved (LMS Synced: 1)
                                </span>
                            ...
                            <div className="flex items-center gap-4 text-xs">
                              <button
                                type="button"
                                onClick={() => openPrivateFile(sub.id)}
                              ...
                              {sub.fileUrls?.additionalLink && (
                                <a
                                  href={sub.fileUrls.additionalLink}
                                  target="_blank"
                                  rel="noopener noreferrer"
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  {lastGrade.grade === 1
                                    ? "Approved"
                                    : "Needs Revision"}{" "}
                                  &bull; <strong>Remarks:</strong> &ldquo;
                                  {lastGrade.cleanRemarks ||
                                    lastGrade.remarks ||
                                    "N/A"}
                                  &rdquo;
                                </div>
                                ...
                                {lastGrade.studentMarks &&
                                  Object.keys(lastGrade.studentMarks).length >
                                    0 && (
                                    <div className="pt-1 border-t border-current/10 flex flex-wrap gap-1.5">
                                      {team.members?.map((m) => {
                                        ...
                                        return (
                                          <span
                                            key={m.id}
                                            className="px-2 py-0.5 rounded bg-white/70 dark:bg-black/20 text-[11px] font-semibold"
                                          >
                                            {
                                              m.student?.user?.name?.split(
                                                " ",
                                              )[0]
                                            }
                                            :{" "}
                            ...
                            {(() => {
                              const mmAssignment =
                                team.examineeAssignments?.find(
                                  (a) => a.phaseId === sub.phaseId,
                                );
                              if (!mmAssignment) return null;
                              const evalsCount =
                                mmAssignment.evaluations?.length || 0;
                              const avgScore =
                                evalsCount > 0
                                  ? (
                                      mmAssignment.evaluations.reduce(
                                        (sum, ev) => sum + ev.totalMarks,
                                        0,
                                      ) / evalsCount
                                    ).toFixed(2)
                                  : "Pending";
                              return (
                                <div className="mt-2 text-xs font-semibold text-indigo-700 bg-indigo-50 dark:bg-indigo-900/30 dark:text-indigo-300 p-2 rounded border border-indigo-100 dark:border-indigo-800 flex items-center gap-1.5">
                                  ...
                                  Avg Score: {avgScore} ({evalsCount} reviews)
                                </div>
                              );
                            })()}
                          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 fade-in">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl w-full max-w-md">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
              Let your students know where and when they can meet you.
            </p>

            <form onSubmit={handleUpdateVenue}>
              <div className="mb-4">
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                  Cabin / Venue Location
                </label>
                <input
                  type="text"
                  value={venue}
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button
                  ...
                  className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold shadow-sm disabled:opacity-50 flex items-center justify-center min-w-[120px]"
                >
            ...
            {interactionTab === "history" ? (
              <div className="space-y-4">
                {interactionLoading ? (
                  ...
                  interactions?.map((interaction) => (
                    <div
                      key={interaction.id}
                      className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-700 space-y-2"
                    >
                      <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400">
                        <span className="font-semibold">
                          Logged by: {interaction.faculty?.user?.name}
                        </span>
                        <span>
              ...
              <form onSubmit={handleLogInteraction} className="space-y-4">
                <p className="text-xs text-gray-500">
                  Record attendance and remarks for each member during this
                  interaction session.
                </p>
                <div className="space-y-3">
                  {interactionTeam.members?.map((member) => (
                    <div
                      key={member.id}
                      className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-700 space-y-2"
                    >
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-sm text-gray-800 dark:text-white">
                          {member.student?.user?.name} (
                          {member.student?.enrollmentNumber})
                        </span>
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={
                              interactionRecords[member.studentId]?.isPresent ||
                              false
                            }
                            onChange={(e) =>
                              setInteractionRecords((prev) => ({
                                ...prev,
                                [member.studentId]: {
                      ...
                      <input
                        ...
                        value={
                          interactionRecords[member.studentId]?.remark || ""
                        }
                        onChange={(e) =>
                          setInteractionRecords((prev) => ({
                            ...prev,
      ...
      {selectedTeamDetails && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold text-gray-800 dark:text-white">
                Team {selectedTeamDetails.teamIdFormatted}
              </h3>
              <button
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl border border-gray-200 dark:border-gray-600 space-y-2">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  <strong>Subject:</strong> {selectedTeamDetails.pbl.subject}{" "}
                  (Sem {selectedTeamDetails.pbl.semester})
                </p>
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  <strong>Leader:</strong>{" "}
                  {selectedTeamDetails.leader?.user?.name}
                </p>
                ...
                {selectedTeamDetails.githubUrl && (
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    ...
                    <a
                      href={selectedTeamDetails.githubUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                ...
                <div className="pt-2 border-t border-gray-200 dark:border-gray-600 flex items-center justify-between text-xs">
                  <span>
                    <strong>Super Mentor:</strong>{" "}
                    {selectedTeamDetails.superMentor?.user?.name ||
                      "Not assigned"}
                  </span>
                  <span className="font-bold">
                    Status: {selectedTeamDetails.superMentorStatus || "PENDING"}
                  </span>
                </div>
                {selectedTeamDetails.superMentorFeedback && (
                  <p className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-2 rounded">
                    <strong>Super Mentor Remarks:</strong>{" "}
                    {selectedTeamDetails.superMentorFeedback}
                  </p>
                )}
              </div>
              ...
              <div>
                <h4 className="font-bold text-gray-800 dark:text-white mb-3">
                  Team Members ({selectedTeamDetails.members.length})
                </h4>
                  ...
                  {selectedTeamDetails.members?.map((member) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-500">
                        Roll No: {member.student?.enrollmentNumber}
                      </p>
                      <p className="text-sm text-gray-500">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  Team {selectedTeamForGrade.teamIdFormatted} &bull; Phase{" "}
                  {selectedSubmission.phase?.phaseNumber || ""}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <form onSubmit={handleGradeSubmit} className="space-y-5">
              ...
              <div>
                ...
                >
                  <option value={1}>
                    Grade 1: Approve Project (Forward to Super Mentor)
                  </option>
                  <option value={0}>
                    Grade 0: Reject Project (Direct Sync to LMS & Ask New Idea)
                  </option>
                </select>
              ...
              <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl border border-gray-200 dark:border-gray-700 space-y-3">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  <div>
                    ...
                    <p className="text-[11px] text-gray-500 dark:text-gray-400">
                      Grade each team member individually based on contribution
                      & viva.
                    </p>
                  </div>
                  {/* Quick Fill Buttons */}
                  <div className="flex items-center gap-1">
                    ...
                    {[10, 9, 8.5, 8].map((score) => (
                      <button
                        ...
                        onClick={() => {
                          const updated = {};
                          selectedTeamForGrade.members.forEach((m) => {
                            updated[m.studentId] = score;
                          });
                ...
                <div className="space-y-2.5 pt-1">
                  {selectedTeamForGrade.members?.map((member) => {
                    const isLeader =
                      selectedTeamForGrade.leaderId === member.studentId;
                    const curMark =
                      studentMarks[member.studentId] !== undefined
                        ? studentMarks[member.studentId]
                        : 10;
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-gray-400">
                            {member.student?.enrollmentNumber} &bull; Sec:{" "}
                            {member.student?.section || "N/A"}
                          </p>
                        </div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setStudentMarks((prev) => ({
                                ...prev,
                                [member.studentId]: isNaN(val)
                                  ? ""
                                  : Math.min(10, Math.max(0, val)),
                              }));
                            }}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={
                    parseInt(grade) === 1
                      ? "e.g. Good progress on Phase objectives. Approved for Super Mentor review."
                      : "e.g. Scope too generic. Please revise your methodology and idea."
                  }
